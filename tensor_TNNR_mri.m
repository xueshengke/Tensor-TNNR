...
admm_result = './result/admm/mri';
apgl_result = './result/apgl/mri';
...
opts.min_R = 10;         % minimum rank of chosen image
...
load('mrdata.mat');
X_full = mrdata(:,:,26:75);
X_full = opts.maxP * (X_full - min(X_full(:))) / (max(X_full(:)) - min(X_full(:)));

[n1, n2, n3] = size(X_full);
fprintf('use MR Brain data: %d x %d x %d ', n1, n2, n3);
